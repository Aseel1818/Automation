...
namespace App74.Services
{
    public class DeleteCourceAdmin
    {
        ...
        {
            var toDeleteOrder = (await client.Child("DrAhmadaccept").OnceAsync<DeleteCourseModel>()).FirstOrDefault(order => order.Object.StudentNumber == studentNumber);

            await client.Child("DrAhmadaccept").DeleteAsync();
